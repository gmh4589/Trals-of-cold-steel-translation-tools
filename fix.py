...
for a in range(len(f_ar)):
	name = 'in/' + f_ar[a] # Изменить путь на свой!!!
	try:
		with open (name) as file:
			file = open(name, 'rb+') 
			new = 'out/' + f_ar[a] # Изменить путь на свой!!!
			fn = open(new, 'wb+')
			file.seek(16, 0)
			start = file.read(4)
			...
			file.seek(20, 0)
			long = file.read(4)
			long = int.from_bytes(long, byteorder='little')
			file.seek(start, 0)
			data = file.read()
			strings = data.split (xmark)
			if (len(strings) > 1):
				file.seek(0, 0)
				if len(f_ar[a]) == 10:
					z = 39
				...
				for i in range(long):
					# if (i > 0):
					ost = len(strings[i]) % 4
					dat = dat + strings[i] + (b'\x00' * ost)
					o = len(dat) + start
					off = o.to_bytes(4, byteorder='little')
					fn.write(off)

				fn.write(dat + strings[long])
					
				print ('Done ' + f_ar[a])
			else:
				print ('Filed ' + f_ar[a])
	except IOError:
		print("File " + f_ar[a] + " dosn't exist!")
		z = 0
