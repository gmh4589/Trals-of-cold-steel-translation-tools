	...
	try:
		with open (name) as file:
			...
			for j in range(a):
				...
				OffsetListOr.append(x)
				
				y = fp.read(4)
				y = int.from_bytes(y, byteorder='little')
				OffsetListTl.append(y)
				
			for k in range(a):
				fo.seek(OffsetListOr[k], 0)
				fp.seek(OffsetListTl[k], 0)
				x1 = fp.read(8)
				x2 = fo.read(8)
				if (x1 != x2):
					table.add_row(f_ar[i], str(OffsetListOr[k]), str(x2), str(OffsetListTl[k]), str(x1))
					error += 1
				
			for l in range(a):
				fp.seek(OffsetListTl[k], 0)
				if ((l+1)<a):
					readc = OffsetListTl[l + 1] - OffsetListTl[l]
				x3 = fp.read()
				ost = len(x3) % 4
				if (ost > 0):
					table.add_row(f_ar[i], str(OffsetListOr[l]), '', str(OffsetListTl[l]), str(ost))
					error += 1
					
			if (error > 0):
				table.add_row(f_ar[i], "Смещений:", str(a), "Ошибок:", str(error))
				table.add_row('----------', '----------', '------------------------------', '----------', '------------------------------')
				errorfiles += 1
	except IOError:
		z = 0
...
table.add_row('')
table.add_row('', "Всего файлов:", str(files), "С ошибками:", str(errorfiles))
console = Console()
console.print(table)
input('Press any key')
